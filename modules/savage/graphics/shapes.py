...
class Text (Grouping, PositionableElement):
    ...
    def createText (self):
        ...
        words = []
        while len (rawWords) > 0:
            words.append (rawWords.pop ())
            if len (rawWords) > 0:
                words.append (' ')
        while len (words) > 0:
            w = words.pop ()
        ...
        if self.direction == 'back':
            self.height = self.y - self.currentLine
        else:
            self.height = self.currentLine - self.y
        self.width = self.maxLen - self.x
        self.applyAnchors ()

    def pop (self, index):
        for i in range (index):
            char = self.children.pop ()
            self.current = char.x
            if char.y > self.currentLine:
                self.numLines -= 1
            self.currentLine = char.y
    ...
    def nextLine (self):
        if self.current > self.maxLen:
            self.maxLen = self.current
        self.current = self.x
        if self.direction == 'forward':
            self.currentLine += self.textHeight
        else:
            self.currentLine -= self.textHeight
        self.numLines += 1
...
LetterDict = {
"&amp;":(667, "M466 78Q455 68 443 56T412 29T371 4T320 -15T257 -23Q165 -23 109 31T52 176Q52 244 86 289T214 389Q167 448 150 482T133 552Q133 618 183 663T304 709T421 665T467 552Q467 498 437 460T329 378L462 215Q493 270 493 330V334H573Q573 240 515 151L637 0H528L466 78ZM285 431Q344 468 366 494T388 554Q388 591 364 615T301 639Q262 639 239 616T215 553Q215 526 227 505T285 431ZM418 137L258 337Q190 294 163 260T136 183Q136 130 174 93T267 55Q340 55 418 137Z"),
"&apos;": (191,"M48 709H142V598L115 464H75L48 598V709Z"),
