...
class BarCanvas (GraphCanvas):
    ...
    def addBar (self, name, val, **attr):
        if not self.padding:
            self.counter += self.graph ().settings.barPadding
            self.padding = True
        #name = self.cleanInput (name)
        settings = self.graph ().settings
        rect = Rectangle (y = 0, x = self.counter, width = 1, height = val)
        self.data.draw (rect)
        rect.xml['has-tooltip'] = True
        rect.xml['name'] = name
        rect.xml['data'] = name
        rect.xml['tooltip-text'] = 'Value: ' + str (val)
        rect.xml['value'] = str (val)
        rect.xml.update (attr)
    ...
    def addGroup (self, group, pairs, url = None):
        if not self.padding:
            self.counter += self.graph ().settings.barPadding
            self.padding = True
        #group = self.cleanInput (group)
        vals = []
        g = Group ()
        g.xml['data'] = group
        settings = self.graph ().settings
        ...
        for i, (key, val) in enumerate (pairs):
            ...
            rect.xml['value'] = str (val)
            if url:
                rect.xml['explore'] = str (url[i])
            self.lastBar = self.counter + settings.barWidth
            self.counter += (settings.barWidth + settings.barSpacing)
        if len (g) > 0:
            g.width = self.lastBar - g.x
            g.height = max (vals)
            self.data.draw (g)
        self.counter += settings.blankSpace
    ...
    def addColor (self, node):
        ...
        try:
            key = node.xml['name']
            node.style.fill = self.colors[key]
        except KeyError:
        ...
        if node.xml.has_key ('has-tooltip'):
            ...
            node.xml['highlight-fill'] = node.style.fill.interpolate (white, .35)

    def barHeights (self):
        for bar in self.data:
            bar.y = self.minY
            bar.height -= self.minY
        #for child in self.data:
        #    child.y = self.minY
        #    child.height -= self.minY

    def finalize (self):
        self.addColor (self.data)
        if len (self.data) > 0:
            self.draw (self.data)
...
class HorizontalBarCanvas (BarCanvas):
    ...
    def setBounds (self):
        ...
        self.minY = 0
        if self.lastBar != 0:
            self.maxY = self.lastBar + self.graph ().settings.barPadding
        else:
            self.maxY = 1
