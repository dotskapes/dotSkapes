...
class Axis (Group):
    def __init__ (self, **attr):
        ...
        if attr.has_key ('textProperties'):
            ...
            self.textPrperties = {}
        if attr.has_key ('has_lines'):
            self.has_lines = attr['has_lines']
        else:
            self.has_lines = False
        if attr.has_key ('line_height'):
            self.line_height = attr['line_height']
        else:
            self.line_height = 0
    ...
    def createTicks (self, tickPos = None):
        ...
        for tick in ticks:
            ...
            if tick == int (tick):
                self.labels.append (str (int (tick)))
            else:
...
class XAxis (Axis):
    ...
    def drawTicks (self):
        for pos, label in zip (self.ticks, self.labels):
            ...
            if self.has_lines:
                l = Line (point1 = (pos, self.y), point2 = (pos, self.y + self.line_height))
                l.style.strokeColor = '#cccccc'
                l.style.strokeWidth = .5
                self.draw (l)
            self.draw (t)
...
class YAxis (Axis):
    ...
    def drawTicks (self):
        ...
        for pos, label in zip (self.ticks, self.labels):
            ...
            if self.has_lines:
                l = Line (point1 = (self.x, pos), point2 = (self.x + self.line_height, pos))
                l.style.strokeColor = '#cccccc'
                l.style.strokeWidth = .5
                self.draw (l)
            self.draw (t)
