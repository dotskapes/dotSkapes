...
class BarGraph (UnifiedGraph):
    ...
    def formatSettings (self, settings):
        ...
        addAttr (settings, 'barPadding', float, 1.0)
    ...
    def addBar (self, name, data, **attr):
        self.canvas.addBar (name, data, **attr)
        #if self.horizontal:
        #    self.ylabels.append (name)

    def addGroup (self, name, data, url = None):
        self.canvas.addGroup (name, data, url)
    ...
    def createYAxis (self):
        if not self.settings.horizontal:
            #for child in self.canvas.data:
            #    self.ypositions.append (child.x + (child.width / 2.0))
